...
class ValueFunction extends Component {
  ...
  renderValueSources = (index) => {
    ...
    const popoverContent = valueSources.filter((valueSource) => valueSource !== 'function').map(srcKey => ({key: `${srcKey + index}`, value: srcKey, label: parseLabelPopover(valueSourcesInfo[srcKey].label) }));
    // let content = (
    //   <RadioGroup
    //     key={valueSrc + '--' + index}
  ...
  renderValueSourceParam = (index, dataTypeOfParam) => {
    ...
    const paramDataType =  value && value.dataTypes[index] || 'text';
    switch (valueSource) {
